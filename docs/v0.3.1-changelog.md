...
├── pyproject.toml         # realm-studio 0.3.1; path dep on realm-fabric
├── backend/
│   ├── app.py             # FastAPI app factory + routes
│   ├── main.py            # ``uv run realm-studio`` entry
...
- `examples/web/realm-studio/tests/test_api.py` — health, snapshot, CRUD POST, turn, prompt, e2e edit+turn (13 tests)
...
| Piece | Detail |
|-------|--------|
| **realm-studio README** | Quick start, full API table, env, test instructions |
| **Root README** | Engine vs apps → realm-studio; V0.3.1 status |
...
| **Package version** | `realm-studio` **0.3.1** in example `pyproject.toml` |
...
| Docs | This changelog, realm-studio README, root README, ROADMAP |
| Tag | **`git tag v0.3.1`** |
