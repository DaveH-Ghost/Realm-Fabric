...
**Baseline:** Builds on **V0.3.0** (`v0.3.0`) — `Session` API, JSON snapshots, `GameProfile`, public `campaign_rpg_engine` package, CLI on Session.

**Overall V0.3.1 focus:** Ship **campaign-rpg-studio**, an example web app that proves the engine works outside the terminal. Primary UX: a **clickable grid** showing agents and objects from `Session.snapshot()`, with **right-click context menus** on empty tiles, agents, and objects to create, edit, or delete entities (via `run_command`). Side panels show passive vision and turn history; **Run turn** drives the LLM for the active agent. The app lives at **`examples/web/campaign-rpg-studio`** in this repo. It is a **reference consumer**, not part of the core library — engine behavior stays in `campaign_rpg_engine` + pytest.
...
| **0.3.1e** | Turn log + passive vision panel | ✅ Implemented |
| **0.3.1f** | Docs, run scripts, optional integration test, tag **`v0.3.1`** | ✅ Implemented |
...
- Custom games can copy `campaign-rpg-studio` without forking `src/main.py`
...
## Layout — `examples/web/campaign-rpg-studio`

```
examples/web/campaign-rpg-studio/
├── README.md              # How to run (dev server, env vars)
├── pyproject.toml         # campaign-rpg-studio 0.3.1; path dep on campaign-rpg-engine
├── backend/
│   ├── app.py             # FastAPI app factory + routes
│   ├── main.py            # ``uv run campaign-rpg-studio`` entry
│   ├── session_store.py   # In-memory Session holder
│   ├── turn_runner.py     # LLM turn (mirrors CLI run)
│   └── schemas.py         # Request bodies
├── frontend/
│   ├── index.html
│   ├── app.js             # Grid + orchestration
│   ├── api.js             # HTTP helpers
│   ├── ui.js              # Context menus, modals
│   ├── panels.js          # Passive vision, turn log
│   └── styles.css
└── tests/
    └── test_api.py        # TestClient smoke + integration (13 tests)
```
...
flowchart LR
    Browser["Browser UI"]
    API["FastAPI backend"]
    Session["Session (campaign_rpg_engine)"]
    LLM["OpenRouter / LLM"]
...
- **Engine API only** in backend imports: prefer `from campaign_rpg_engine import Session, load_profile, AgentCompoundTurn`.
...
```text
┌─────────────────────────────────────────────────────────────┐
│  campaign-rpg-studio     Active: [Explorer ▼]        [Run turn ▶]  │
├──────────────────────────────┬──────────────────────────────┤
│                              │  Passive Vision              │
...
| Piece | Detail |
|-------|--------|
| **`examples/web/campaign-rpg-studio/`** | `pyproject.toml` (path dep on `campaign-rpg-engine`), README |
...
| **Dev server** | `uv run campaign-rpg-studio` → http://127.0.0.1:8765 |
| **Minimal frontend** | JSON snapshot viewer (`/` + `/static/*`) — grid UI in **0.3.1b** |

### Run

```powershell
cd examples\web\campaign-rpg-studio
uv sync
...
- `examples/web/campaign-rpg-studio/tests/test_api.py` — health, snapshot, CRUD POST, turn, prompt, e2e edit+turn (13 tests)
...
| Piece | Detail |
|-------|--------|
| **Passive vision panel** | Renders `snapshot.passive_vision` for the active agent |
| **Turn log** | Appends composite result + step summaries after each successful **Run turn** |
| **Session turn counter** | Labeled “Session turn” in sidebar meta + header status line |
| **Debug prompt viewer** | Collapsible panel; shows last turn prompt or fetches `GET /api/prompt` |
| **`GET /api/prompt`** | Builds current compound prompt for active (or `agent_id`) agent |
| **`frontend/panels.js`** | Passive vision, turn log, prompt debug helpers |

### Out

- Persistent turn log across page reload (in-memory for v1)
- Full prompt editor
...
**Goal:** Someone can clone the repo, run campaign-rpg-studio, and play the demo room in a browser.

### Shipped

| Piece | Detail |
|-------|--------|
| **campaign-rpg-studio README** | Quick start, full API table, env, test instructions |
| **Root README** | Engine vs apps → campaign-rpg-studio; V0.3.1 status |
| **ROADMAP** | V0.3.1 marked complete; V0.3.2+ notes (sprites, GM events) |
| **Integration test** | `test_e2e_edit_then_turn` — command + mocked turn |
| **Package version** | `campaign-rpg-studio` **0.3.1** in example `pyproject.toml` |
| **Tag** | **`git tag v0.3.1`** when ready (example milestone; engine stays `0.3.0`) |
...
| Step | Detail |
|------|--------|
| Example app | `examples/web/campaign-rpg-studio` runs locally |
| Engine dep | `campaign-rpg-engine>=0.3.0` (path dep in dev) |
| Tests | Root `uv run pytest` green; example 13 TestClient tests green |
| Docs | This changelog, campaign-rpg-studio README, root README, ROADMAP |
| Tag | **`git tag v0.3.1`** |
| Proof | Grid with right-click create/edit/delete + **Run turn** end-to-end |
...
- If the example needs a small engine hook, add it to `campaign_rpg_engine` in a **`0.3.0.x`** patch with tests, not in the example only.
