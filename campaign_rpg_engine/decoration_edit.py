...
from campaign_rpg_engine.area_edit import slugify_display_name
...
def _remove_background_decorations(area: Area) -> None:
    area.decorations[:] = [
        d for d in area.decorations if d.kind != DECORATION_KIND_BACKGROUND
    ]
...
def add_decoration_to_area(
    ...
    *,
    ...
    if normalized_kind == DECORATION_KIND_BACKGROUND:
        ...
        dec_id = decoration_id or generate_decoration_id(
            area, _decoration_id_label(image, label)
        )
